...
function highlightBtn(btns, index) {
  btns.forEach((b, i) => {
    b.style.outline = i === index ? '2px solid #ff6633' : 'none';
    b.style.outlineOffset = i === index ? '2px' : '0';
  });
}
...
export function show(container) {
          ...
          container.innerHTML = `
            <div class="ui-screen" style="justify-content:center;">
              ...
              <div class="ui-container ui-text" style="max-width:400px;">
                ...
                <button class="ui-btn ui-btn--secondary" id="stats-back" style="margin-top:16px;">BACK</button>
              </div>
            </div>
          `;
          const statsBack = container.querySelector('#stats-back');
          statsBack.addEventListener('click', () => show(container));
          // Gamepad B to go back from stats
          const statsGp = startGamepadNav({
            onBack: () => { statsGp(); show(container); },
            onConfirm: () => { statsGp(); show(container); }
          });
        });
        return;
      case 'online-create':
  ...
  const onKey = (e) => {
    if (e.code === 'ArrowUp' || e.code === 'KeyZ') {
      ...
      activateBtn(btns[focusIndex]);
    }
  };
  window.addEventListener('keydown', onKey);

  // Gamepad nav
  const stopGamepad = startGamepadNav({
    onUp: () => {
      focusIndex = (focusIndex - 1 + btns.length) % btns.length;
      highlightBtn(btns, focusIndex);
    },
    ...
    onConfirm: () => activateBtn(btns[focusIndex])
  });
